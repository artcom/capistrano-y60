...
configuration.load do
  ...
  Comment=kills the watchdog & y60
    ...
    desc "Setup http_proxy"
    task :set_http_proxy, :roles => :app do
      next if find_servers_for_task(current_task).empty?
      run "echo 'export http_proxy=192.168.193.53:8888' | #{sudo} tee /etc/profile.d/autoset_http_proxy.sh", :pty => true
    end
